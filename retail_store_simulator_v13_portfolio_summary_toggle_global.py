...
# Initialize per-tenant state once
...
def compute_financials(cfg):
    ...
    months_in_year = [5, 12, 12, 12, 12, 7]  # fixed as per v13
    ...
    for i, months in enumerate(months_in_year):
        ...
        revenue = store_revenue * (months / 12.0)
    ...
    df["Cumulative Profit (د.إ)"] = np.cumsum(df["Profit (د.إ)"]).tolist()

    # extremums & payback
    peak_year = int(df.loc[df["Profit (د.إ)"].idxmax(), "Financial Year"])
    worst_year = int(df.loc[df["Profit (د.إ)"].idxmin(), "Financial Year"])
    payback_year = next((int(y) for y, cp in zip(df["Financial Year"], df["Cumulative Profit (د.إ)"]) if cp >= 0), None)
...
st.set_page_config(page_title="Retail Lease Simulator - V13 (Portfolio)", layout="wide")
st.title("🏬 Retail Store Lease Profitability Simulator (v13) — Portfolio-Linked Summary")

# ------------------------------
# Sidebar: Tenant selector + sliders (persist per tenant)
# ------------------------------
with st.sidebar:
    ...
    # Collect user inputs for the selected tenant (reflect current state values)
    plot_area_sqm = st.number_input("Plot Area (sqm)", value=float(cfg["plot_area_sqm"]), step=10.0, key=f"plot_area_{tenant_name}")
    lease_years = int(cfg["lease_years"])  # fixed in original; no widget
    base_rent = st.number_input("Annual Rent (د.إ)", value=float(cfg["base_rent"]), step=1000.0, key=f"base_rent_{tenant_name}")
    rent_escalation = st.slider("Rent Escalation (%/yr)", 0.0, 10.0, float(cfg["rent_escalation"]), key=f"rent_esc_{tenant_name}")
    service_charge = st.number_input("Service Charges (د.إ/sqft)", value=float(cfg["service_charge"]), step=0.5, key=f"service_{tenant_name}")
    ...
    uae_avg_revenue = st.slider("UAE Avg Revenue (د.إ)", 20_000_00, 80_000_00, int(cfg["uae_avg_revenue"]), step=1_000_00, key=f"uae_avg_rev_{tenant_name}")
    ...
    # Update current tenant's state from widgets
    ...
    # (a) Apply rent-free months to ALL tenants
    ...
    # (b) Copy ALL current tenant settings to ALL tenants
...
# ------------------------------
# Compute per-tenant and portfolio metrics live
# ------------------------------
tenant_results = {}
...
# Selected tenant details (for Dashboard tab)
curr = tenant_results[tenant_name]
df = curr["df"]
k = curr["kpis"]
years = curr["years"]

# Build portfolio (sum across tenants year-wise)
lease_years = 6
portfolio_df = pd.DataFrame({"Financial Year": [2024 + i for i in range(lease_years)]})
for col in ["Revenue (د.إ)", "Base Rent (د.إ)", "Service Charges (د.إ)", "Other Costs (د.إ)", "Profit (د.إ)"]:
...
portfolio_df["Margin (%)"] = np.where(portfolio_df["Revenue (د.إ)"] > 0,
                                      (portfolio_df["Profit (د.إ)"] / portfolio_df["Revenue (د.إ)"]) * 100.0, 0.0)
portfolio_df["Cumulative Profit (د.إ)"] = np.cumsum(portfolio_df["Profit (د.إ)"]).tolist()

# Portfolio KPIs
pt_total_profit = portfolio_df["Profit (د.إ)"].sum()
pt_total_revenue = portfolio_df["Revenue (د.إ)"].sum()
pt_total_rent = portfolio_df["Base Rent (د.إ)"].sum()
pt_total_service = portfolio_df["Service Charges (د.إ)"].sum()
pt_total_costs = portfolio_df["Other Costs (د.إ)"].sum()
pt_roi = round((pt_total_profit / pt_total_costs) * 100.0, 2) if pt_total_costs else 0.0
pt_avg_margin = (portfolio_df["Profit (د.إ)"].sum() / pt_total_revenue * 100.0) if pt_total_revenue else 0.0
pt_peak_year = int(portfolio_df.loc[portfolio_df["Profit (د.إ)"].idxmax(), "Financial Year"])
pt_worst_year = int(portfolio_df.loc[portfolio_df["Profit (د.إ)"].idxmin(), "Financial Year"])
pt_payback_year = next((int(y) for y, cp in zip(portfolio_df["Financial Year"], portfolio_df["Cumulative Profit (د.إ)"]) if cp >= 0), None)
...
with tab_dash:
    ...
    col1.metric("💼 Total Profit", f"د.إ {k['total_profit']:,.0f}")
    col2.metric("📊 Avg Profit Margin", f"{k['avg_margin']:.1f}%")
    col3.metric("🪙 Total ROI", f"{k['lease_roi']:.1f}%")

    # Financial Table (selected tenant)
    st.markdown("### 📘 Financial Year-wise Breakdown — Selected Tenant")

    df_display = df.copy()
    currency_cols = ["Revenue (د.إ)", "Base Rent (د.إ)", "Service Charges (د.إ)", "Other Costs (د.إ)", "Profit (د.إ)", "Cumulative Profit (د.إ)"]
    ...
    margin_fig.add_trace(go.Bar(x=years, y=df["Base Rent (د.إ)"], name="Base Rent", yaxis='y'))
    margin_fig.add_trace(go.Bar(x=years, y=df["Service Charges (د.إ)"], name="Service Charges", yaxis='y'))
    margin_fig.add_trace(go.Bar(x=years, y=df["Other Costs (د.إ)"], name="Other Costs", yaxis='y'))
...
with tab_exec:
    ...
    if scope == "Selected Tenant":
        ...
        c1.metric("Total Revenue", f"د.إ {sel_k['total_revenue']:,.0f}")
        c2.metric("Total Rent (All Years)", f"د.إ {sel_k['total_rent']:,.0f}")
        c3.metric("Total Service Charges", f"د.إ {sel_k['total_service']:,.0f}")
        c4.metric("Total Other Costs", f"د.إ {sel_k['total_costs']:,.0f}")

        c5, c6, c7 = st.columns(3)
        c5.metric("Total Profit", f"د.إ {sel_k['total_profit']:,.0f}")
        c6.metric("Avg Margin", f"{sel_k['avg_margin']:.1f}%")
        c7.metric("ROI on Costs", f"{sel_k['lease_roi']:.1f}%")

        st.markdown("---")
        st.subheader("Cumulative Profit Trajectory (Payback) — Selected Tenant")
        cum_fig = go.Figure()
        cum_fig.add_trace(go.Scatter(
            x=sel_df["Financial Year"],
            y=sel_df["Cumulative Profit (د.إ)"],
            mode="lines+markers",
            name="Cumulative Profit"
        ))
        cum_fig.add_hline(y=0, line_width=1, line_dash="dash")
        if sel_k["payback_year"] is not None:
            cum_fig.add_vline(x=sel_k["payback_year"], line_width=1, line_dash="dot",
                              annotation_text=f"Payback Year {sel_k['payback_year']}", annotation_position="top")
        cum_fig.update_layout(height=420, title="Cumulative Profit Over Lease Term — Selected Tenant")
        st.plotly_chart(cum_fig, use_container_width=True)

        st.markdown("---")
        st.subheader("Cash Flow View — Selected Tenant")
        cash_df = sel_df[["Financial Year", "Revenue (د.إ)", "Base Rent (د.إ)", "Service Charges (د.إ)", "Other Costs (د.إ)", "Profit (د.إ)", "Cumulative Profit (د.إ)"]].copy()
        ...
        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Total Revenue", f"د.إ {pt_total_revenue:,.0f}")
        c2.metric("Total Rent (All Years)", f"د.إ {pt_total_rent:,.0f}")
        c3.metric("Total Service Charges", f"د.إ {pt_total_service:,.0f}")
        c4.metric("Total Other Costs", f"د.إ {portfolio_df['Other Costs (د.إ)'].sum():,.0f}")

        c5, c6, c7 = st.columns(3)
        c5.metric("Total Profit", f"د.إ {pt_total_profit:,.0f}")
        c6.metric("Avg Margin (Weighted)", f"{pt_avg_margin:.1f}%")
        c7.metric("ROI on Costs", f"{pt_roi:.1f}%")

        st.markdown("---")
        st.subheader("Portfolio Payback & Cumulative Profit")
        cum_fig = go.Figure()
        cum_fig.add_trace(go.Scatter(
            x=portfolio_df["Financial Year"],
            y=portfolio_df["Cumulative Profit (د.إ)"],
            mode="lines+markers",
            name="Cumulative Profit"
        ))
        cum_fig.add_hline(y=0, line_width=1, line_dash="dash")
        if pt_payback_year is not None:
            cum_fig.add_vline(x=pt_payback_year, line_width=1, line_dash="dot",
                              annotation_text=f"Payback Year {pt_payback_year}", annotation_position="top")
        cum_fig.update_layout(height=420, title="Cumulative Profit Over Lease Term (Portfolio)")
        st.plotly_chart(cum_fig, use_container_width=True)
        ...
        for tname, res in tenant_results.items():
            ...
            snap_rows.append({
                "Tenant": tname,
                "Total Revenue (د.إ)": kx["total_revenue"],
                "Total Rent (د.إ)": kx["total_rent"],
                "Service (د.إ)": kx["total_service"],
                "Other Costs (د.إ)": kx["total_costs"],
                "Total Profit (د.إ)": kx["total_profit"],
