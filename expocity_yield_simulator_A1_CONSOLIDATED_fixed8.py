...
st.set_page_config(page_title="Expo‑city Yield Optimizer — Dual Mode v6.19", layout="wide")
...
if mode == "Macro (Footfall & costs)":
    ...
    with t1:
        ...
        fig1.add_trace(go.Scatter(x=YEARS, y=total_rev, name="Total revenue", mode="lines",
                                  customdata=np.array(lock_labels)[:,None],
                                  hovertemplate="Year %{x}<br>Total revenue: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"))
        fig1.add_trace(go.Scatter(x=YEARS, y=cur["expenses"], name="Costs", mode="lines",
                                  customdata=np.array(lock_labels)[:,None],
    ...
    with t2:
        ...
        # --- Compute portfolio aggregates BEFORE showing metrics ---

        total_rev = np.zeros(len(YEARS)); total_exp = np.zeros(len(YEARS)); total_val = np.zeros(len(YEARS))

        total_base = np.zeros(len(YEARS)); total_turnover = np.zeros(len(YEARS))
        ...
        for n in t_names:
            ...
            # Build per‑tenant row for the selected focus year

            rows.append({

                "Tenant": n,

                "Category": st.session_state["tenants"][n]["category"],

                "RF (mo)": st.session_state["tenants"][n]["rf_months"],

                "Property value (AED)": r["value"][focus_idx],

                f"{focus_lbl} revenue (AED)": r["revenue"][focus_idx],

                f"{focus_lbl} costs (AED)": r["expenses"][focus_idx],

                f"{focus_lbl} yield %": ((r["revenue"][focus_idx] - r["expenses"][focus_idx]) / max(1e-9, r["value"][focus_idx])) * 100.0,

                f"Store sales {focus_lbl} (AED)": (r["turnover"][focus_idx] / max(1e-9, (st.session_state["tenants"][n]["turnover_pct_internal"] / 100.0))),

                "Turnover on sales %": st.session_state["tenants"][n]["turnover_pct_internal"],

            })
        stats_slot = st.container()

        mix_slot = st.container()
        ...
        with stats_slot:
                    ...
                    # Headline portfolio metrics for the selected year
                    ...
                    try:
                        ...
                        _n = max(1, (len(YEARS)-1 - focus_idx))

            

                        port_cagr_focus = ((total_rev[-1] / max(1e-9, total_rev[focus_idx])) ** (1.0/_n) - 1.0)
        ...
        if any(st.session_state.get(f"syncfs_{name}", False) for name in t_names):
            pass
        if st.session_state.get("lock_fy24", True):
            pass
        ...
        for n in t_names:
            ...
            rows.append({
                ...
                "Category": st.session_state["tenants"][n]["category"],
                "RF (mo)": st.session_state["tenants"][n]["rf_months"],
                "Property value (AED)": r["value"][0],
                "FY24 revenue (AED)": r["revenue"][0],
                "FY24 costs (AED)": r["expenses"][0],
                "FY24 yield %": ((r["revenue"][0] - r["expenses"][0]) / max(1e-9, r["value"][0])) * 100.0,
                "Store sales FY24 (AED)": (r["turnover"][0] / max(1e-9, (st.session_state["tenants"][n]["turnover_pct_internal"] / 100.0))),
                "Turnover on sales %": st.session_state["tenants"][n]["turnover_pct_internal"],
                "YoY revenue growth %": ((r["revenue"][1] / max(1e-9, r["revenue"][0])) - 1.0) * 100.0
            })
        ...
        with mix_slot:
                    ...
                    figp1.add_trace(go.Scatter(x=YEARS, y=total_rev, name="Total revenue", mode="lines",

                                               customdata=np.array(lock_labels_p)[:,None],

                                               hovertemplate="Year %{x}<br>Total revenue: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"))

                    figp1.add_trace(go.Scatter(x=YEARS, y=total_exp, name="Costs", mode="lines",
        ...
        st.dataframe(
        pt_df.style.format({
            "Property value (AED)": "{:,.0f}",
            "Store sales FY24 (AED)": "{:,.0f}",
            "Turnover on sales %": "{:.0f}",
            "FY24 revenue (AED)": "{:,.0f}",
            "FY24 costs (AED)": "{:,.0f}",
            "FY24 yield %": "{:.0f}",
            "YoY revenue growth %": "{:.0f}"
    ...
    with t4:
        ...
        if st.session_state["scenarios"]:
            ...
            # === Column filtering & ordering (focus-year + FY29 extras + existing growth column) ===
            footfall_label = f"Footfall {focus_lbl}"
            base_label = f"{focus_lbl} base rent (AED)"
            turnover_label = f"{focus_lbl} turnover share (AED)"
            revenue_label = f"{focus_lbl} revenue (AED)"
            costs_label = f"{focus_lbl} costs (AED)"
            yield_label = f"{focus_lbl} yield %"
            pv_label = f"{focus_lbl} portfolio value (AED)"
            
            # Find the existing annual growth column if present (e.g., "Annual growth rate % (FY25→FY29)")
            growth_col = None
            for _c in comp.columns:
                _cl = _c.lower()
                if "annual growth rate" in _cl and "fy" in _cl and "fy29" in _cl:
                    growth_col = _c
                    break
            
            # Identify FY29 extras if present
            def _find_fy29(substr):
                for c in comp.columns:
                    cl = c.lower()
                    if "fy29" in cl and substr in cl:
                        return c
                return None
            fy29_rev_col = _find_fy29("revenue")
            fy29_yield_col = _find_fy29("yield")
            
            # Optional helpers to find Footfall growth % and YoY growth columns
            def _find_col(substrs):
                for c in comp.columns:
                    ...
                    if all(s in cl for s in substrs):
                        return c
                return None
            footfall_growth_col = _find_col(["footfall", "growth"])
            yoy_growth_col = _find_col(["yoy", "growth"]) or _find_col(["yoy growth"]) or _find_col(["growth", "yoy"])
            
            # Keep only focus-year core columns, inflation, capture, footfall growth, YoY growth,
            # the EXISTING annual growth column, and the two FY29 extras.
            core_keep = {footfall_label, base_label, turnover_label, revenue_label, costs_label, yield_label}
            always_keep_prefixes = ["capture"]
            always_keep_exact = {"Inflation %"}
            optional_keep = {c for c in [footfall_growth_col, yoy_growth_col, growth_col] if c}
            fy29_keep = {c for c in [fy29_rev_col, fy29_yield_col] if c}
            
            def _should_keep(col):
                cl = col.lower()
                if col in core_keep or col in always_keep_exact or col in optional_keep or col in fy29_keep:
                    return True
                if any(pref in cl for pref in always_keep_prefixes):
                    return True
                if col == pv_label:
                    return False
                if "fy" in cl and focus_lbl.lower() not in cl:
                    return col in fy29_keep
                return False
            comp = comp[[c for c in comp.columns if _should_keep(c)]]
            
            # === Column ordering per your request ===
            preferred = [base_label, turnover_label, footfall_label]
            if footfall_growth_col and footfall_growth_col in comp.columns:
                preferred.append(footfall_growth_col)
            preferred += ["Inflation %", revenue_label, costs_label, yield_label]
            if yoy_growth_col and yoy_growth_col in comp.columns:
                preferred.append(yoy_growth_col)
            # Append FY29 extras at the end (revenue then yield), then the EXISTING annual growth column (last)
            for extra in [fy29_rev_col, fy29_yield_col, growth_col]:
                if extra and extra in comp.columns and extra not in preferred:
                    preferred.append(extra)
            ordered = [c for c in preferred if c in comp.columns]
            leftovers = [c for c in comp.columns if c not in ordered]
            comp = comp[ordered + leftovers]
            
            # Ensure growth column is formatted as percentage (if formatting lists exist later)
            try:
                pct_cols = [*pct_cols, growth_col] if growth_col else pct_cols
            except NameError:
                pass
    ...
    # >>> FS_EXACT_SECTION_BEGIN
    # FS: three placeholders that are cleared on every rerun of this section
    if "_fs_ph" not in st.session_state:
        st.session_state["_fs_ph"] = (st.empty(), st.empty(), st.empty())
    else:
        for _p in st.session_state["_fs_ph"]:
            _p.empty()
    ph1, ph2, ph3 = st.session_state["_fs_ph"]
    ...
    with ph1.container():
        ...
        fig1.add_trace(go.Bar(
            x=YEARS, y=lease_rental, name="Lease rental income",
            customdata=np.array(lock_labels)[:, None],
            hovertemplate="Year %{x}<br>Lease rent: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"
        ))
        fig1.add_trace(go.Bar(
            x=YEARS, y=turnover_rev, name="Turnover revenue",
            customdata=np.array(lock_labels)[:, None],
            hovertemplate="Year %{x}<br>Turnover: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"
        ))
        fig1.add_trace(go.Scatter(
            x=YEARS, y=revenue, name="Total revenue", mode="lines",
            customdata=np.array(lock_labels)[:, None],
            hovertemplate="Year %{x}<br>Total revenue: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"
        ))
        fig1.add_trace(go.Scatter(
            x=YEARS, y=expenses, name="Costs", mode="lines",
            customdata=np.array(lock_labels)[:, None],
            hovertemplate="Year %{x}<br>Costs: %{y:,.0f} AED<br>%{customdata[0]}<extra></extra>"
        ))
        ...
        if be:
            fig1.add_trace(go.Scatter(
                x=[be["x"]], y=[be["y"]], mode="markers+text",
                marker=dict(color="red", size=12),
                text=[f"BE {be['fy']}"], textposition="top center",
                hovertemplate=("Breakeven at %{customdata[0]}<br>%{customdata[1]}: "
                               "%{customdata[2]:.2f} of year<br>Revenue = Costs = %{y:,.0f} AED<extra></extra>"),
                customdata=[[be["date"], be["fy"], be["t"]]],
                name="Breakeven", showlegend=True, line=dict(width=0)
            ))
    
        fig1.update_layout(barmode="stack", height=380, yaxis_title="AED", margin=dict(t=60))
        add_fy24_shading(fig1, lock_fy24)
        st.plotly_chart(fig1, use_container_width=True, key="fs_rev_mix")  # stable key
    
    # --- FS chart 2 ---
    with ph2.container():
        st.markdown("### Revenue each year + cumulative — exact (AED)")
        ...
        fig2.add_trace(go.Scatter(
            x=YEARS, y=cum_rev, name="Cumulative (AED)", mode="lines", yaxis="y2"
        ))
    ...
    # --- FS chart 3 ---
    with ph3.container():
        st.markdown("### Operating costs & yield/breakeven — exact")
        ...
        if 'add_fy24_shading' in globals():
            add_fy24_shading(fig3p, lock_fy24)
